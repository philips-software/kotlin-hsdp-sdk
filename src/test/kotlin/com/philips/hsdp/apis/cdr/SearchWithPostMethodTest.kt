...
internal class SearchWithPostMethodTest : CdrTestBase() {

    private val searchMethod = SearchMethod.Post
    ...
    ).map { (format, expectedBody) ->
        DynamicTest.dynamicTest("Format value $format should lead to query parameter $expectedBody") {
            runBlocking {
                // Given
                server.enqueue(mockSuccessResponse)
    ...
    fun `Should include the query parameters when providing them`(): Unit = runBlocking {
        ...
        // Then
        request.body.readUtf8() shouldBe "k=v"
    }
